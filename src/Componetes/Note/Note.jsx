...
export default function Note({noteDetails,deleteNote , getUserNotes}) {
  ...
  return (
    ...
    <div className="col-md-4">
        <div className='bg-white note p-4 m-2 rounded-2 shadow-sm'>
            <h3 className='text-secondary fw-bold'>{noteDetails.title}</h3>
            <p>{noteDetails.desc}</p>
        <div>
            <i onClick={()=>{deleteNote(noteDetails._id)}}  className="fa-solid del fa-trash-can pe-2"></i>
            <i onClick={handleShow}  className="fa-solid fa-pen-to-square edit"></i>
            </div>
        </div>
    </div>
    </>
